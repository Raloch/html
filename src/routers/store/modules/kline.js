...
class Kline {
  ...
  widgetInit = () => {
    ...
    this.widget = window.tvWidget = new window.TradingView.widget({
      ...
      width: '99%',
      height: '100%',
      symbol: trade.currentCoinsType, // 'ADABTC'
      interval: '15',
    ...
    this.widget.onChartReady(() => {
      ...
      this.widget.chart().createStudy("Moving Average", false, false, [7], null, {"Plot.linewidth": 2,"plot.color": "#ffba70"});
      this.widget.chart().createStudy("Moving Average", false, false, [30], null, {"Plot.linewidth": 2,"plot.color": "#6bcaed"})
      // 离开页面时需要移除监听，否则会报错
      document.addEventListener('click', this.refreshKline, false)
    })
    // 创建自定义按钮 -- 深度图
    this.widget.headerReady().then(() => {
      let button = this.widget.createButton({ align: 'right' })
      button.setAttribute('title', '深度图')
      button.setAttribute('class', 'button-depth')
      button.textContent = '深度图'
      button.addEventListener('click', function() {
        trade.kline.isShowDepth = true
      })
    })
  }
  @action refreshKline = () => {
    this.widget.chart().setSymbol(trade.currentCoinsType, data => {
      // console.log('k线数据刷新')
    })
  }
  // 将周期转换成秒
  @action timeConversion = time => {
    switch(time) {
      case ('1M' || '1W' || '1D' || 'D'):
        return 86400
      case '240':
  ...
  // 发送历史数据请求
  @action sendKlineQueryReq = params => {
    let data = {
      id: 8,
      method: 'kline.query',
      params: params
