...
class Trade {
  ...
  @action updateMarket = res => {
    ...
    // 最近成交
    if (home.urlpath === '/trade' && data.method === 'deals.update') {
    ...
    // 深度
    if (home.urlpath === '/trade' && data.method === 'depth.update') {
      // count为20, 50
    ...
    // k线 -- 实时数据
    if (home.urlpath === '/trade' && typeof data.ttl === 'number') {
    ...
    // k线 -- 历史数据
    if (home.urlpath === '/trade' && data.method === 'kline.update') {
...
// 侦听 -- trade.depth.count，值改变则出发函数（页面初始渲染时不触发）（autorun初始渲染时会触发）
...
const reaction2 = reaction(
  () => trade.currentCoinsType,
  type => {
    trade.sendReq(2, 'depth.subscribe', [`${ type }`, parseFloat(trade.depth.count), '0.00000001'])
  }
)
