...
@inject('store')
@observer
class GoogleChangeOne extends Component {
    constructor(){
        super()
    }
    state={
    ...
    changePhoNext = (e) => {
        let _this=this
        let googleCode = this.props.form.getFieldValue('changeGoogleInputCode')
        let phoneCode = this.props.form.getFieldValue('changePhoInputCode')
        ...
        CgicallPost("/apiv1/visitor/validateCodeExist",obj,function(d){
            if(d.result) {
                ...
                sessionStorage.emailChangeCode = changeEmailCode || '';
                sessionStorage.phoneChangeCode = phoneCode || '';
                sessionStorage.googleChangeCode = googleCode || '';
                //初始化session
                sessionStorage.gooBoundCode = '';
                sessionStorage.inputEmailCode = '';
                sessionStorage.phoneCode = '';
                // _this.props.history.push({ pathname :'/users/userCenter/googleTrans' ,change : {changeSuc:true,emailChangeCode: changeEmailCode,phoneChangeCode:phoneCode,googleChangeCode:googleCode}})
                _this.props.history.push({ pathname :'/users/userCenter/googleTrans'})
...
export default Form.create()(GoogleChangeOne);